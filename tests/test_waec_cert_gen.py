...
def test_monument_template_registered_and_renders(app):
    assert 'monument' in W.TEMPLATES and 'monument' in W._CANVAS_DRAW
    assert W.TEMPLATES['monument']['name'] == 'Monument — 2026'
    assert W.TEMPLATES['monument']['landscape'] is False
    assert 'monument' in W.PRESETS
    sid = _seed(app)
    with app.app_context():
        ctx = W.build_context(db.session.get(Student, sid), _YR)
        show = W.preset_show(ctx, 'monument')
        assert show['subjects'] and show['grades'] and show['total_subjects']
        assert ctx['stats']['a1'] == 5 and ctx['stats']['credits'] == 9
        pdf = W.render_pdf(ctx, 'monument', show, verify_url='https://example.test/verify/ABC')
        assert pdf.getvalue()[:4] == b'%PDF'
        import fitz
        doc = fitz.open(stream=pdf.getvalue(), filetype='pdf')
        assert round(doc[0].rect.width) == 595 and round(doc[0].rect.height) == 842
