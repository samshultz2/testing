...
def test_generated_site_is_image_rich(app):
    ...
    with app.app_context():
        ...
        home = SitePage.query.filter_by(slug='home').first()
        hero = next(b for b in home.blocks if b['type'] == 'hero')
        # hero uses an image-bearing variant with real imagery
        assert hero['variant'] in ('image-bg', 'split', 'image-right', 'slider')
        assert (hero['props'].get('bg_image') or hero['props'].get('image')
                or hero['props'].get('slides'))
...
def _hero_img(props):
    return props.get('bg_image') or props.get('image') or (props.get('slides') or [None])[0]
...
def test_two_schools_get_different_photos(app):
    ...
    with app.app_context():
        site_generator.generate()
        a_img = _hero_img(next(b for b in SitePage.query.filter_by(slug='home').first().blocks
                               if b['type'] == 'hero')['props'])
    _named(app, 'Sunrise International College')
    with app.app_context():
        site_generator.generate()
        b_img = _hero_img(next(bl for bl in SitePage.query.filter_by(slug='home').first().blocks
