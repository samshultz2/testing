...
class WAECAnalytics(ExamAnalytics):
    """Comprehensive WAEC results analytics"""
    ...
    def get_year_statistics(year):
        ...
        student_metrics = []
        for student_id, student_res in student_results.items():
            credits = sum(1 for r in student_res if r.grade in ['A1', 'B2', 'B3', 'C4', 'C5', 'C6'])
            a1s = sum(1 for r in student_res if r.grade == 'A1')
            distinctions = sum(1 for r in student_res if r.grade in ['A1', 'B2', 'B3'])
            total_points = sum(ExamAnalytics.WAEC_GRADE_POINTS.get(r.grade, 9) for r in student_res)
            
            student_metrics.append({
                'student_id': student_id,
                'subjects': len(student_res),
                'credits': credits,
                'a1s': a1s,
                'distinctions': distinctions,
                'total_points': total_points,
                'avg_points': round(total_points / len(student_res), 2) if student_res else 0
            })
        
        # Overall statistics
        all_credits = [m['credits'] for m in student_metrics]
        all_a1s = [m['a1s'] for m in student_metrics]
        all_avg_points = [m['avg_points'] for m in student_metrics]
        
        stats = {
            'year': year,
            'unique_students': len(student_results),
            'total_results': len(results),
            
            # Credit statistics
            'total_credits': sum(all_credits),
            'avg_credits': round(sum(all_credits) / len(all_credits), 1) if all_credits else 0,
            'max_credits': max(all_credits) if all_credits else 0,
            'min_credits': min(all_credits) if all_credits else 0,
            
            # A1 statistics
            'total_a1s': sum(all_a1s),
            'avg_a1s': round(sum(all_a1s) / len(all_a1s), 1) if all_a1s else 0,
            'max_a1s': max(all_a1s) if all_a1s else 0,
            
            # Points statistics (lower is better)
            'avg_points': round(sum(all_avg_points) / len(all_avg_points), 2) if all_avg_points else 0,
            'best_avg_points': min(all_avg_points) if all_avg_points else 0,
            
            # Thresholds
            'with_5_credits': sum(1 for m in student_metrics if m['credits'] >= 5),
            'with_6_credits': sum(1 for m in student_metrics if m['credits'] >= 6),
            'with_7_credits': sum(1 for m in student_metrics if m['credits'] >= 7),
            'with_8_credits': sum(1 for m in student_metrics if m['credits'] >= 8),
            'with_9_credits': sum(1 for m in student_metrics if m['credits'] >= 9),
        }
        
        # Percentages
        total = len(student_metrics) or 1
        stats['pct_5_credits'] = round((stats['with_5_credits'] / total) * 100, 1)
        stats['pct_6_credits'] = round((stats['with_6_credits'] / total) * 100, 1)
        
        # Grade distribution across all results
        grade_dist = defaultdict(int)
        for r in results:
            grade_dist[r.grade] += 1
        stats['grade_distribution'] = dict(grade_dist)
        
        # Pass rate (credits / total)
        total_credit_grades = sum(1 for r in results if r.grade in ['A1', 'B2', 'B3', 'C4', 'C5', 'C6'])
        stats['overall_pass_rate'] = round((total_credit_grades / len(results)) * 100, 1) if results else 0
        stats['overall_distinction_rate'] = round(
            (sum(1 for r in results if r.grade in ['A1', 'B2', 'B3']) / len(results)) * 100, 1
        ) if results else 0
    ...
    def get_subject_performance(year):
        ...
        subject_data = defaultdict(list)
        for r in results:
            subject_data[r.subject].append(r.grade)
        
        subject_stats = []
        for subject, grades in subject_data.items():
            a1 = sum(1 for g in grades if g == 'A1')
            b2_b3 = sum(1 for g in grades if g in ['B2', 'B3'])
            credits = sum(1 for g in grades if g in ['A1', 'B2', 'B3', 'C4', 'C5', 'C6'])
            fails = sum(1 for g in grades if g in ['D7', 'E8', 'F9'])
            ...
            stats = {
                ...
                'a1': a1,
                'b2_b3': b2_b3,
                'distinctions': a1 + b2_b3,
                'credits': credits,
                'fails': fails,
                'pass_rate': round((credits / len(grades)) * 100, 1) if grades else 0,
                'distinction_rate': round(((a1 + b2_b3) / len(grades)) * 100, 1) if grades else 0,
                'avg_points': round(sum(points) / len(points), 2) if points else 0,
                'grade_distribution': {g: grades.count(g) for g in set(grades)}
            }
            subject_stats.append(stats)
        
        return sorted(subject_stats, key=lambda x: x['pass_rate'], reverse=True)
    
    @staticmethod
    def get_top_performers(year, limit=10):
        """Get top performing students by total credits and avg points"""
        results = WAECResult.query.filter_by(exam_year=year).all()
        ...
        for student_id, student_res in student_results.items():
            ...
            credits = sum(1 for r in student_res if r.grade in ['A1', 'B2', 'B3', 'C4', 'C5', 'C6'])
            a1s = sum(1 for r in student_res if r.grade == 'A1')
            points = sum(ExamAnalytics.WAEC_GRADE_POINTS.get(r.grade, 9) for r in student_res)
            avg_points = points / len(student_res) if student_res else 9
        ...
        # Sort by credits desc, then avg_points asc
        metrics.sort(key=lambda x: (-x['credits'], x['avg_points']))
