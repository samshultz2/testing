...
class ProductionConfig(Config):
    ...
    # Support impersonation works under Strict because establish() lands on a page
    # on the school's OWN host that then steps into the portal — that final request
    # is same-origin, so the just-set cookie is always sent (no reliance on a
    # cross-context redirect carrying it).
